    ...
    if(Command[1] == 'i' || Command[1] == 's' || Command[1] == 'g' || Command[1] == 'm')
    {
        ...
        switch (Command[1])
        {
            ...
            case 'i':
                    //将字符串转为整形
                    //测试过一组数据
                    //串口输入:-i 192.168.178.100
                    //value_Network_Setting的值为: 192 168 178 100
            ...
            case 'm':
                    //将十六进制字符串转换为整形
                    //测试过一组数据:
                    //串口助手输入:-m 94:54:CE:2A:13:75
                    //value_Network_Setting的值对应是: 148 84 206 42 19 117
